                ...
                <div className="flex items-center gap-3">
                    <div className="flex bg-muted rounded-lg p-1">
                        <button onClick={() => setViewMode('board')}
                            className={cn('px-3 py-1.5 rounded-md text-sm font-medium transition-colors', viewMode === 'board' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground')}>
                            Board
                        </button>
                        <button onClick={() => setViewMode('list')}
                            className={cn('px-3 py-1.5 rounded-md text-sm font-medium transition-colors', viewMode === 'list' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground')}>
                            List
                        </button>
                    </div>
                            ...
                            <div key={status} className="bg-card/50 border border-border rounded-xl p-4">
                                ...
                                <div className="space-y-3">
                                    {groupedTasks[status].length === 0 ? (
                                        ...
                                        <div key={task._id} className="bg-card border border-border rounded-lg p-3.5 hover:border-primary/30 transition-colors">
                                            ...
                                            <div className="flex gap-1 mt-3 pt-3 border-t border-border">
                                                {columns.filter(s => s !== status).map(s => (
                                                    <button key={s} onClick={() => handleStatusChange(task._id, s)}
                                                        className="text-xs px-2 py-1 rounded bg-muted hover:bg-muted/80 text-muted-foreground transition-colors flex-1">
                                                        {STATUS_LABELS[s]}
                                                    </button>
                                                ))}
                                            </div>
                            ...
                            <tbody>
                                {tasks.map(task => (
                                    <tr key={task._id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                                        ...
                                        <td className="p-4">
                                            <span className={cn('text-xs px-2.5 py-1 rounded-full font-medium', PRIORITY_STYLES[task.priority])}>
                                                {task.priority}
                                            </span>
                                        </td>
                                        <td className="p-4">
                                            <select value={task.status} onChange={(e) => handleStatusChange(task._id, e.target.value)}
                                                className="text-xs px-2 py-1 rounded-lg bg-muted border border-border focus:outline-none">
                                                {columns.map(s => <option key={s} value={s}>{STATUS_LABELS[s]}</option>)}
                                            </select>
                                        </td>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-card border border-border rounded-2xl p-6 w-full max-w-lg animate-fade-in">
                        ...
                        <form onSubmit={handleCreate} className="space-y-4">
                            {createError && (
                                <div className="p-3 bg-destructive/10 border border-destructive/20 rounded-xl text-destructive text-sm">{createError}</div>
                            )}
                            <div>
                                <label className="text-sm font-medium text-foreground mb-1 block">Project *</label>
                                <select value={form.projectId} onChange={(e) => setForm({ ...form, projectId: e.target.value })}
                                    className="w-full px-4 py-3 bg-muted/50 border border-input rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary" required>
                            ...
                            <div className="flex justify-end gap-3 pt-2">
                                <button type="button" onClick={() => setShowCreate(false)}
                                    className="px-5 py-2.5 bg-muted text-foreground rounded-xl font-medium hover:bg-muted/80 transition-colors">Cancel</button>
                                <button type="submit" disabled={creating}
                                    className="px-5 py-2.5 bg-primary text-primary-foreground rounded-xl font-medium hover:bg-primary/90 disabled:opacity-50 transition-colors flex items-center gap-2">
