...
import {
    ...
    Menu, X, LogOut, ChevronLeft, ChevronRight, Zap, ListTodo,
} from 'lucide-react'
    ...
    return (
        <div className="min-h-screen flex bg-background">
            ...
            <aside className={cn(
                ...
                <div className={cn('h-16 flex items-center border-b border-border/30 px-4', sidebarOpen ? 'justify-between' : 'justify-center')}>
                    {sidebarOpen ? (
                    ...
                    <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-2 rounded-lg hover:bg-sidebar-hover transition-colors hidden lg:flex">
                        {sidebarOpen ? <ChevronLeft className="w-5 h-5" /> : <ChevronRight className="w-5 h-5" />}
                    </button>
                    <button onClick={() => setMobileMenuOpen(false)} className="p-2 rounded-lg hover:bg-sidebar-hover transition-colors lg:hidden">
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-3 border-t border-border/30">
                    {sidebarOpen ? (
                        <div className="flex items-center gap-3 px-3 py-2">
                            ...
                            <button onClick={logout} className="p-2 rounded-lg hover:bg-sidebar-hover transition-colors" title="Logout">
                                <LogOut className="w-5 h-5" />
                            </button>
                        </div>
                    ) : (
                        <button onClick={logout} className="w-full flex justify-center p-2 rounded-lg hover:bg-sidebar-hover transition-colors" title="Logout">
                            <LogOut className="w-5 h-5" />
                        </button>
                    )}
                </div>
            ...
            <div className="flex-1 flex flex-col min-h-screen">
                <header className="h-16 bg-card border-b border-border flex items-center px-4 lg:px-6 sticky top-0 z-30">
                    <button onClick={() => setMobileMenuOpen(true)} className="p-2 rounded-lg hover:bg-muted transition-colors lg:hidden">
